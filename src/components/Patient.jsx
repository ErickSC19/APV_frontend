...
const Patient = ({ patient }) => {
  const { setToEdit } = usePatients();
  const { email, name, owner, date, syntoms } = patient;
  ...
  return (
    <div className='mx-5 my-10 border-2 border-dashed px-5 py-10 rounded-md grid grid-cols-3'>
      ...
      <div className='flex flex-col justify-between'>
        <button type='button' className='py-2 px-10 bg-red-600 hover:bg-red-700 text-white transition-colors rounded-lg uppercase font-bold'>Delete</button>
